' EOS (hiResPeek) foot shadow editor launcher (port 8765)
...
If appDir = "" Then
    MsgBox "Kit folder not found." & vbCrLf & vbCrLf & _
        ...
        "under: " & base, vbCritical, "EOS shadow editor"
    WScript.Quit 1
End If
...
Sub OpenEosShadowQuiet(sh)
    Dim url, xhr, ready, i
    url = "http://127.0.0.1:8765/eos-shadow"
    ready = False
    WScript.Sleep 3200
    For i = 1 To 8
        On Error Resume Next
